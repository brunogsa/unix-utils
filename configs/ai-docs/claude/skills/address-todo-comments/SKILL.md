...
- `TODO` / `XXX` — legacy markers; either a question or an action.
  - Infer which from the comment text and its surrounding code/prose.
  - When genuinely ambiguous, batch every ambiguous one into a single clarifying round with the user instead of guessing marker by marker.
...
2. **Gather every marker with `grep -n`, then read each hit's surrounding context directly.**
   - Search the target(s) for `AI!`, `AI?`, `TODO`, `XXX` literally.
...
4. **Create one TaskList task per cluster** you identified — don't file a task per raw marker if several belong to one theme.
...
5. **Execute, then strip the marker.**
...
This skill targets loose `AI?`/`AI!`/`TODO`/`XXX` comment markers, in code or prose.
