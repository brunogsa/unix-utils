#!/usr/bin/env bash
# extract-md-sections - Print selected level-2 (##) sections
# from a Markdown file.
#
# Pulls each named "## <section>" heading plus its body —
# everything up to the next "## " heading or EOF — and prints
# the kept sections in file order.
#
# Built for embedding chosen slices of spec_<slug>.md /
# plan_<slug>.md into a PR description without hand-copying,
# since the whole file usually blows GitHub's 65536-char body
# cap (see the sibling check-pr-body-size.sh).
#
# Content before the first kept heading (title, preamble) is
# dropped, as are any "## " sections you don't name.
#
# Nested "### " sub-headings ride along inside their parent
# section untouched.
#
...
#      stderr)
#
# A requested title that isn't found prints a non-fatal
# warning on stderr, so typos surface without aborting the
# run.
#
# Examples:
#   extract-md-sections.sh spec_sge.md "Background / Context" \
#     "Testable Acceptance Criteria" > slice.md
#
#   extract-md-sections.sh plan_sge.md "Test Design" \
#     "Technical Decisions"
#
...
# Warn (non-fatal) on any title that doesn't exist as a
# whole-line "## <title>".
for title in "$@"; do
...
# Build a newline-separated list of the exact heading lines to
# keep, passed via the environment rather than `awk -v`.
#
# Some awk implementations (e.g. macOS's onetrueawk) fail to
# parse a -v value containing literal newlines.
wanted=""
for title in "$@"; do
    wanted+="## $title"$'\n'
done
