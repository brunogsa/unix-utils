...
Using `config` instead of `configuration` will cause the viewer to show empty/zero values.

Putting `pass_rate` at the top level of a run instead of nested under `result` will also cause the viewer to show empty/zero values.
