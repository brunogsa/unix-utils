...
def evaluate_name(path, lexicon):
    ...
    # Path.stem strips only the final extension, so a suite
    # named "foo.test.py" has stem "foo.test" — the literal "."
    # trips the kebab-case check below even when "foo" itself is
    # fine.
    #
    # A test suite is not exempt from the naming rule; it is
    # judged on its real stem, with the ".test" marker set aside
    # first.
    stem = path.stem
