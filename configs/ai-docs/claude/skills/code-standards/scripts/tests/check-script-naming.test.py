...
class TestCheckScriptNamingHappy:
    ...
    def test_should_not_flag_a_skill_directory_that_only_occurs_as_a_substring_of_a_longer_token(self, tmp_path):
        # The 'pr' skill directory occurs inside 'preview' as a
        # character run, not as a repeated token — 'preview' !=
        # 'pr'.
        # A substring test wrongly matches 'pr' in 'preview';
        #
        # the rule is about a repeated whole token in the
        # hyphen-split name.
        script = _write(
    ...
    def test_should_pass_a_name_built_on_the_capture_verb(self, tmp_path):
        # 'capture' is a genuine imperative verb distinct from
        # every other lexicon entry — this locks in that the
        # lexicon recognizes it, not just that some other rule
        # stays silent.
    ...
    def test_should_pass_a_name_built_on_the_verb_for_writing_to_a_remote(self, tmp_path):
        # Every other lexicon verb reads or transforms locally,
        # so a script that writes to a remote had no verb it
        # could be named after.
        #
        # And a checker with no word for what a script does
        # pressures the author to rename a precise name into a
        # vague one.
        #
        # 'post' closes that hole for every such script, not
...
class TestCheckScriptNamingFailure:
    def test_should_fail_a_name_whose_first_token_is_not_a_recognized_verb(self, tmp_path):
        # The unknown-verb rule is what most of the live corpus
        # fails on, yet every other test here reaches it only
        # incidentally (via a name that also trips some other
        # rule).
        #
        # This pins the rule on its own.
        #
        # 'frobnicate' is absent from the lexicon's verb list,
        # while 'density' and 'report' are past the len<=3
        # abbreviation gate and absent from the category
        # denylist.
        #
        # So the unknown verb is the only rule this name can
        # trip.
    ...
    def test_should_fail_a_name_that_separates_its_words_with_anything_but_hyphens(self, tmp_path):
        # Underscored names are the live corpus's real non-kebab
        # shape (estimate_tokens.sh, parse_ics.py).
        #
        # Uppercase would fail the same single character-class
        # clause in KEBAB_RE, so it is the same cause, not a
        # second one worth its own test.
    ...
    def test_should_report_only_the_kebab_case_reason_when_a_name_is_not_kebab_case(self, tmp_path):
        # evaluate_name returns immediately on a kebab-case
        # failure, because every later rule reads hyphen-split
        # tokens that a non-kebab stem cannot produce
        # meaningfully.
        #
        # 'check_density' splits into one token that is neither
        # a verb nor an object, so continuing would stack
        # invented reasons onto a name whose single real defect
        # is its separator.
    ...
    def test_should_fail_a_name_whose_object_is_a_category_rather_than_a_specific_thing(self, tmp_path):
        ...
        # "shared" is 6 characters, well past the abbreviation
        # branch's own len<=3 gate, so it can never also be
        # flagged as an abbreviation.
        #
        # This is the true invariant the abbreviation loop's
        # now-removed (dead) denylist re-check never changed.
    ...
    def test_should_fail_a_name_that_repeats_a_multi_word_skill_directory_token_by_token(self, tmp_path):
        # The skill directory itself is hyphenated
        # ('open-in-tmux').
        #
        # The repeat check must match its tokens as a contiguous
        # run in the stem, not just a single token — otherwise a
        # script literally named after its multi-word skill
        # slips through.
        script = _write(
    ...
    def test_should_not_flag_ordinary_short_english_words_as_abbreviations(self, tmp_path, script_name):
        # Live false positives from the L3 finding: 'to', 'add',
        # 'and', 'by' are whole words, not abbreviations.
        #
        # Every other token here is a recognized verb or a
        # >3-char object, so the abbreviation branch is the only
        # rule these names could still trip.
    ...
    def test_should_not_flag_the_atlassian_document_format_acronym_as_an_abbreviation(self, tmp_path):
        # 'adf' is the Atlassian Document Format — a format
        # acronym with no spelled-out form anyone writes,
        # exactly like the already-allowlisted 'dag'.
        #
        # Naming the format is what makes such a script's name
        # precise, so the allowlist has to carry it or the rule
        # pushes the author toward a vaguer name.
        #
        # 'build' is a recognized verb and 'payload' is past the
        # len<=3 gate, so the abbreviation branch is the only
        # rule this name could still trip.
...
class TestCheckScriptNamingStandingExclusions:
    ...
    def test_should_skip_the_vendored_eval_viewer_review_generator(self, tmp_path):
        # Excluded as a single named file rather than by
        # directory, so the exclusion cannot swallow a sibling
        # script that this repo does own.
    ...
    def test_should_still_report_a_badly_named_sibling_of_the_vendored_review_generator(self, tmp_path):
        # Guards the exclusion above from widening into its
        # directory: only generate_review.py is vendored, so
        # anything else under eval-viewer/ must still be judged.
    ...
    def test_should_exempt_the_named_bootstrap_entrypoints_away_from_a_tree_root(self, tmp_path, entrypoint):
        # These two are exempted by basename wherever they
        # appear, not by sitting at a tree root — the sibling
        # tree-mode test pins the root case, and this pins that
        # the exemption is not secretly position-dependent.
...
class TestCheckScriptNamingTestSuiteStem:
    def test_should_pass_a_test_suite_whose_underlying_stem_is_valid_kebab_case(self, tmp_path):
        # Regression this guards: path.stem on
        # "check-density.test.py" is "check-density.test"
        # (Path.stem strips only the final ".py"), and the
        # literal "." inside that stem trips KEBAB_RE.
        #
        # A false positive on every *.test.py suite in this
        # repo, regardless of whether its real stem is valid.
    ...
    def test_should_still_fail_a_test_suite_whose_underlying_stem_violates_the_naming_rule(self, tmp_path):
        # A *.test.py suite is not exempt from the naming rule.
        #
        # Its real stem (with the trailing ".test" stripped)
        # must still obey it, and the reported reason must be a
        # real rule violation ("object"), never "name is not
        # kebab-case".
...
class TestCheckScriptNamingRootLevelScripts:
    def test_should_report_a_badly_named_script_sitting_directly_at_the_tree_root(self, tmp_path):
        # Regression this guards: collect_tree_scripts used to
        # skip any file with fewer than 2 relative path parts
        # under tree_root, silently exempting every root-level
        # script from tree mode regardless of name.
        #
        # A corpus's bootstrap/ entrypoint scripts are the
        # common case, but an arbitrary badly-named root-level
        # script must still surface.
    ...
    def test_should_still_exempt_install_sh_and_run_tests_sh_at_the_tree_root(self, tmp_path):
        # install.sh and run-tests.sh are conventional
        # bootstrap/ entrypoint names, exempted by basename
        # (mirroring classify-conversion.py's is_excluded)
        # rather than by a blanket "anything at root" rule.
        #
        # The blanket rule also hid genuinely bad root-level
        # names (see the sibling test).
        repo = tmp_path / "myrepo"
...
class TestCheckScriptNamingStaleWorktreeExclusion:
    def test_should_scan_a_corpus_nested_under_a_worktrees_ancestor_that_is_not_the_stale_checkout(self, tmp_path):
        ...
        # it (exit 0, no output).
        #
        # -- indistinguishable from "fully compliant".
        #
        # Only the one named checkout,
        # worktrees/stacked-prs-pr2, is stale; a sibling
        # worktree directory holds a live corpus that must still
        # be swept.
...
class TestCheckScriptNamingRealCorpusRegression:
    ...
    def test_should_report_exactly_the_30_oh_my_zsh_commands_lib_and_root_scripts_known_to_fail_naming(self):
        ...
        #
        # 27 commands/+lib/ paths plus 3 root-level scripts
        # (Scout #89: collect_tree_scripts no longer
        # blanket-skips anything sitting directly at tree_root).
        #
        # Only the named install.sh/run-tests.sh entrypoints
        # stay exempt, and oh-my-zsh's install.sh is the only
        # one of those two present at this root.
        #
        # commands/render-ascii-mermaid.sh dropped out of this
        # set once the shared naming-rule-lexicon.json learned
        # the verb 'render' (unix-utils commit 1c48a12).
        #
        # The lexicon is shared across the whole five-repo
        # stack, so a verb added for one repo's script can
        # legitimately un-fail another repo's script that used
        # the same verb correctly.
        ...
        # Every other discovered script in this corpus must
        # still fail naming -- render-ascii-mermaid.sh is the
        # one legitimate pass now that the lexicon knows
        # 'render'.
    ...
    def test_should_sweep_the_live_unix_utils_tree_without_reporting_nothing_at_all(self):
        # unix-utils is the corpus several agents commit to at
        # once, so its exact failing set is other people's
        # in-flight work, not a property of the checker.
        #
        # What stays true no matter who adds or renames a script
        # is that a real tree of this depth yields *some*
        # classified script — the assertion below is therefore
        # stable by construction, where an exact set is not.
        #
        # The rules deciding OK vs FAIL are pinned over the
        # synthetic corpus in the fixture classes above, which
        # no concurrent commit can move.
        ...
        # Drift stays visible as a warning rather than a
        # failure.
        #
        # The committer who turns this set red is usually not
        # the one who changed it, and a red shared suite they
        # cannot act on trains everyone to green it by editing
        # the expectation.
