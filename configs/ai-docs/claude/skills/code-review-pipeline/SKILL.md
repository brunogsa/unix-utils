...
Implementation details for both modes: see [`references/wave5-emit.md`](./references/wave5-emit.md).
...
- **No findings at all**:
  ...
  - LOCAL: write `${out_file}` (the timestamped `./report_auto-review_<timestamp>` file) with "no findings" under Findings.
