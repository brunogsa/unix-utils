...
def brainstorm():
    ...
    #      each task's acceptance criteria in its Task Details entry against
    #      its Tests (planned) list.
    #      ALWAYS at light, the library's "How would this break?" judgment,
    #      over each task's Task Details acceptance criteria — at full
