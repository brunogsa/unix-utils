...
After ALL approved fixes are applied (not after each one — at the end of the batch), run every gate the project exposes, in parallel where independent. Review-driven fixes look mechanical but renames, contract changes, removed code, and type-narrowing tweaks all surface failures here — not at edit time.
...
Discover the actual commands from the repo (`package.json` scripts, `Makefile`, `justfile`, repo CLAUDE.md). Prefer the project's "agentic" / "ci" variants when they exist. If a check doesn't exist for this project, say so explicitly instead of skipping silently.
...
**Fix what you find.** Any failure your fix caused is part of the fix — resolve it before declaring done. Pre-existing failures your branch did NOT introduce are reported AND filed: per CLAUDE.md's hard-contract Scout rule, **every distinct pre-existing finding gets a TaskCreate in the same response** that surfaces it (cycles, failing tests, skipped/disabled tests, dead-code findings in untouched modules, etc.). Filing is non-negotiable; whether to execute later is the user's call.
