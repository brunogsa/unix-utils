...
Load [`references/flow-ordering.md`](references/flow-ordering.md) whenever a reader could otherwise follow the wrong edge first — it covers the `classDef start` entry-point highlight and the numbering scheme for multi-step flows.

It also carries the don't-over-color rule and routes on to the lifecycle markers for to-be / migration diagrams.
...
Data model at entity level — entities, their keys, and cardinality. Entity names must be real table names, per Naming above.

Load [`references/diagram-erd.md`](references/diagram-erd.md) when drawing one — covers the cardinality glyphs and which side each describes.

It also carries identifying vs non-identifying lines, the mandatory-label parse trap, and the attribute-scope rule.
