...
**Find the root cause before proposing a fix.** Symptom-fixes mask defects, create new ones, and waste time. Even simple bugs have root causes — the process is fast for small bugs and irreplaceable for large ones.
...
1. **Web search the exact symptom** — error message, stack-trace fragment, library name + version, framework + error code. Someone else has likely hit this; their answer beats a fourth guess. Use the WebSearch tool.
...
3. **Question the architecture, not just the code.** If each fix reveals a new symptom in a different place, the *pattern* is broken — not the implementation. Surface the pattern to the user before continuing.
