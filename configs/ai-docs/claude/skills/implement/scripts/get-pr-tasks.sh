#!/usr/bin/env bash
# get-pr-tasks.sh - resolves a PR Breakdown's PR-N label to its
# listed task-id list.
#
# Usage:
#   get-pr-tasks.sh <plan-file> <PR-N>
#
# Extracts the "## PR Breakdown" section from <plan-file>, finds
# the entry labeled <PR-N> (e.g.
# "PR-2"), and prints its "Tasks: <N, N>" clause verbatim.
#
# The printed format is the same comma-space "N, N" list
# /implement's own <task-ids> argument already expects (see
# implement/SKILL.md's Usage section), so /implement's
# pre-flight can pipe this straight through.
#
# Exit codes:
...
# entry-parse pipeline shared with need-git-checkout.sh and
# check-pr-dependencies-ready.sh; this script only consumes the
# Tasks: field (column 2) of its TSV output.
#
...
# Looked up via a plain string match (never a failing awk exit
# code).
#
# Under `set -e`, a command substitution's failure inside a
# bare assignment (not an if/while/&&/|| condition) aborts the
# script before the diagnostic below can print, so "not found"
# must surface as an empty string, not a non-zero exit.
