...
# nested "#### Corner cases" / "#### Failure modes" groups.
...
# taxonomy_file - a small, fixed corner-cases/failure-modes
# taxonomy (with an Async delivery row nested under Failure
# modes, mirroring the real file's structure) so fixtures stay
# short and immune to drift in the real canonical taxonomy.
...
# write_spec - writes a reshaped spec fixture (stub AC section,
# then "# Appendix" and "## Coverage Checklists").
#
# The two checklist bodies come from the caller.
write_spec() {
  local name="$1" boundary_body="$2" failure_body="$3"
  local path="$work_dir/$name.md"
  {
    printf '## Testable Acceptance Criteria\n\n'
    printf '### AC-1: When a request arrives, the system shall respond\n\n'
    printf -- '- **When** a request arrives\n- **Then** it succeeds\n\n'
    printf '# Appendix\n\n## Coverage Checklists\n\n'
    printf '### Boundary checklist\n\n%s\n\n' "$boundary_body"
    printf '### Failure category checklist\n\n%s\n' "$failure_body"
  } > "$path"
  printf '%s' "$path"
}

# write_spec_no_appendix - a fixture with no "# Appendix" /
# "## Coverage Checklists" section at all.
write_spec_no_appendix() {
  local name="$1"
  local path="$work_dir/$name.md"
...
it_should_fail_when_one_taxonomy_row_is_missing_from_a_checklist() {
  local fixture
  fixture=$(write_spec "missing-row" \
    '- empty / single / many: covered (AC-2)' \
    '- validation error (4xx): covered (AC-3)
- downstream timeout / never-responds: N/A — irrelevant
- duplicate delivery (at-least-once redelivery): N/A — irrelevant')
...
it_should_fail_when_a_row_has_malformed_grammar() {
  local fixture
  fixture=$(write_spec "malformed-row" \
    '- empty / single / many: covered (AC-2)
...
it_should_pass_trivially_when_the_spec_has_no_checklist_block_at_all() {
  local fixture
  fixture=$(write_spec_no_appendix "no-checklist")
  run_script "$fixture"
