...
Read alongside `plan-writing.md` when writing the Test Design, Task Breakdown, PR Breakdown, or Decision logs sections of a plan.
...
Group each `describe()` block by scenario class, one comment per class, e.g.:
...
Commit sketch line: `` `<repo>` — `type(scope): subject` `` — repo path, Conventional Commits subject.
