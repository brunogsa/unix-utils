...
- **Scope, don't blind**: hand each subagent gate the changed hunks plus the full doc.
  ...
  - The deterministic script gates ("Every test has a task", and the mechanical half of "Every AC has a test") are exempt from scoping.
...
- **Re-check broken invariants**: each gate concentrates on the changed regions plus any invariant those changes break, even in UNCHANGED regions.
  - Deletions are the trap: removing an AC orphans the plan machinery tracing to it (the "Every line traces to an AC" toggle).

  - Removing a task orphans its owned test title ("Every test has a task").

  - Both orphans sit in unchanged regions the diff won't flag — this is what the full-doc backstop must catch.
  - The local case is easier: an edited AC or test re-runs that AC↔test coverage pair ("Every AC has a test").
