...
# step queries via `git ls-files` / `git check-ignore`.
#
# The hook reads its cwd via os.getcwd(), so run_hook below
# cd's into this dir for every invocation rather than passing
# cwd as a parameter.
sandbox_dir=""
...
setup_sandbox() {
  ...
  (
    ...
    # Untracked-and-not-ignored fixture, reused by the "still
    # BLOCKED" tests below: this is the exact case the guard
    # exists to catch (real work with exactly one copy on disk,
    # no git recovery).
...
# Bug 1 (real, hit live this session): split_segments() splits
# the ENTIRE raw command on newlines with no awareness that
# lines inside a `<<'EOF' ... EOF` heredoc body are literal
# data, not separate shell segments.
#
# A commit message body that merely mentions "rm -f" in prose
# (describing a past incident) shattered into many
# unparseable-looking segments.
#
# The final fail-closed regex then found "rm" inside that
# inert body text and blocked a command that never actually
# invokes rm.
...
# Bug 2 (real, hit live this session): split_segments() splits
# on a bare `|` with no quote-awareness, so an
# escaped pipe INSIDE a quoted string (e.g. a grep alternation
# pattern) still gets treated as a shell pipe operator.
#
# That shatters `grep -i "git-guard\|rm-guard"` into two
# segments, each with an unbalanced quote (unparseable).
#
# The final fail-closed regex then matched "rm" inside the
# substring "rm-guard" (a bare `\b` treats
# `-` as a word boundary), blocking a command that
# never invokes rm at all.
...
# Regression guard: the heredoc-stripping fix for Bug 1 must not
# weaken the fail-closed property for a genuinely dangerous,
# genuinely unparseable rm.
#
# Here the REAL `rm -f` invocation is on the heredoc
# OPENER line (kept, not stripped) and that opener line itself
# has an unbalanced quote — so this must still be blocked after
# the fix.
...
# Bug 3 (Scout #45, real, hit live this session):
# split_segments() splits the raw command text on
# separators (`;`, `|`, `&`, `&&`, `||`, newline).
#
# It does this with no quote-awareness.
#
# The plain text "rm -rf" alone, with no separator-shaped
# character alongside it, never actually shattered
# split_segments() — this case already passed before the fix and
# is kept here as a regression guard, not a bug demonstration.
#
# The genuine break needs a separator character (`;`,
# `|`, ...) sitting INSIDE the quotes too, which is
# what the next test reproduces.
...
# The actual reproduction of bug 3: the "rm -rf" text alone (no
# shell metacharacter alongside it) never tripped split_segments
# — it takes a separator-shaped character (here, `;`)
#
# sitting INSIDE the quotes to make the quote-unaware regex
# split cut the command in two mid-string, which is what turned
# this into an unparseable, fail-closed block live this session.
...
# The quoted "rm" in the grep clause must stay inert data (bug 3
# above);
#
# the real rm after && must still be recognized and blocked (its
# target is the untracked, not-ignored fixture from
# setup_sandbox — no git copy).
