...
# the file listing the checkers scan.
new_repo() {
...
# These are the user's own doc-standards, so only
# markdown in the repo the hook itself lives in is
# theirs to hold to them.
#
...
it_should_stay_silent_in_a_repo_the_installed_hook_does_not_gate() {
  ...
  append_wide_line "$repo" tracked.md
  local t; t=$(write_transcript "$repo/transcript.jsonl" "$repo/tracked.md")
  run_hook_at "$HOOK" "$repo" "{\"session_id\":\"foreign-repo-case\",\"stop_hook_active\":false,\"transcript_path\":\"$t\"}"
...
it_should_still_block_when_run_from_a_copy_seated_in_the_repo_it_gates() {
  ...
  local t; t=$(write_transcript "$repo/transcript.jsonl" "$repo/tracked.md")
  run_hook_at "$seated" "$repo" "{\"session_id\":\"seated-copy-case\",\"stop_hook_active\":false,\"transcript_path\":\"$t\"}"
...
it_should_stay_silent_when_the_running_hook_sits_outside_any_git_work_tree() {
  ...
  local t; t=$(write_transcript "$repo/transcript.jsonl" "$repo/tracked.md")
  run_hook_at "$loose" "$repo" "{\"session_id\":\"unseated-hook-case\",\"stop_hook_active\":false,\"transcript_path\":\"$t\"}"
...
it_should_still_block_when_invoked_by_a_relative_path_from_a_subdirectory() {
  ...
  run_hook_at "../hooks/$(basename "$HOOK")" "$repo/sub" \
    "{\"session_id\":\"relative-invocation-case\",\"stop_hook_active\":false,\"transcript_path\":\"$t\"}"
