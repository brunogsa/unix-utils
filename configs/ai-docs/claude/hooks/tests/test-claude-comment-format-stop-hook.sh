...
# for, skip-drops, a stale delegate line staying inert,
# a mixed skip+new file set, and the
...
#
# The Scout's description has to carry the checker's
# own --fix command: that deterministic pass used to
# run from the approved branch this change removes, so
# without it whoever runs the Scout starts by hand.
