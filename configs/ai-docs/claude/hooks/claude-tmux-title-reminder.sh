...
Always pass ONLY the current work (the compact-focus label), never a composed title. Once this session has survived a compaction the script prefixes the title it froze beforehand, rendering "<root>/<compact-focus>[M+S]" inside a wider 24-character cap -- M is this session's own compactions, S is subagent compactions (S is dropped entirely when zero) -- so never type a "/" or a "[M+S]" yourself, and never try to restate the original title from memory. Post-root, your compact-focus label gets its own fixed room -- at most 8 characters including the leading "/", so effectively up to 7 -- and that room shrinks further as the [M+S] counter widens, so keep it as short as the pre-compaction 16-char title. The script owns that composition; handing it "<root>/<compact-focus>" would nest a second root inside the first.
