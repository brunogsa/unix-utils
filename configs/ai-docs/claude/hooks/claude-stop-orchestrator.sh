...
#
#   Sequencing them here fixes it: run the density gate first; only if it does
#   NOT block (the turn is genuinely over) do we run the notification. One ping,
#   on the real stop.
#
# This does NOT merge the two scripts:
#   claude-density-stop-hook.sh and claude-tmux-notification.sh stay standalone,
#   independently testable, and usable on their own events (the notification is
#   still wired directly to the Notification event elsewhere). This orchestrator
#   only sequences them for the Stop event and gates the notification on the
#   gate's verdict.
#
...
#   - A missing or failing density gate fails OPEN: treat as "no block" and
#     notify — better a stray ping than a swallowed stop.
#   - Block detection is by the gate's JSON output, not its exit code (the gate
...
# 3. Gate is clean → this is the real stop → fire the "done" notification.
