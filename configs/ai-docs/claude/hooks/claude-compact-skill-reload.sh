#!/usr/bin/env bash
# claude-compact-skill-reload - After a compaction, remind
# Claude to RELOAD any still-needed skills it loaded earlier
# this session.
#
# Usage (Claude Code SessionStart hook, matcher: compact):
#   stdin:  hook event JSON ({ session_id, transcript_path,
#           source, ... })
#
#   stdout: optional JSON ({ hookSpecificOutput: {
#           additionalContext } })
#
# Rationale:
...
#   doing" failure.
#
#   The session transcript is append-only across compaction,
#   so every Skill tool call this session made is still on
#   disk even once summarized out of context.
#
#   This hook greps the transcript for those loads, dedups
#   them.
#
#   It re-injects the list with a directive to reload the
#   still-necessary ones FIRST -- before continuing the task
#   -- so the procedure is re-established at the one moment
...
#
#   Complements claude-implement-compact-reminder.sh: that
#   hook reconstructs /implement's exact remaining finalize
#   steps from its state file; this one is the general net
#   for every other skill.
#
# Why this hook skips a subagent's compaction:
#   The directive reloads skills the MAIN session loaded
#   before this compaction -- a subagent's own transcript
#   never had them.
#
#   agent_id (falling back to agent_type) scopes the reminder
#   to the owning session only. Same precedent as
#   claude-explore-mandate-hook.sh and
...
#
# - No Skill loads found in the transcript -> exit 0 (no
#   noise).
#
...
# A Skill tool call logs as one JSONL line carrying both
# `"name":"Skill"` and `"input":{"skill":"<name>"}`.
#
# Filter to those lines FIRST so a Bash command or tool result
# that merely mentions the `"skill":"..."` pattern (e.g. this
# session's own greps) can't masquerade as a real load.
#
# Then pull the names, dedup, and join as ", " for a readable
# list.
