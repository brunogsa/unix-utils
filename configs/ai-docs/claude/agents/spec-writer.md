...
4. The `design-docs` skill (Skill tool) — ownership + altitude rules that keep the spec from re-deriving durable ADR/HLD/LLD content. Load once, on a write call only.
