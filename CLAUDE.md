...
The committed values are the declared defaults: `model` `sonnet`, `effortLevel` `high`, `advisorModel` `opus`. Restore them any time with `git checkout -- configs/ai-docs/claude/settings.json`.
